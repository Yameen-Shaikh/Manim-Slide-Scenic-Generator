...
class SlideLayouts:
    ...
    def render_hero(self, data):
        ...
        img = self.slide.get_img(data.get("image")).scale(Theme.IMAGE_SCALE_HERO).shift(DOWN * Theme.SHIFT_DEFAULT)
    ...
    def render_comparison(self, data):
        ...
        l_img = self.slide.get_img(data.get("left_img")).scale(1).shift(LEFT * 3.5 + DOWN * Theme.SHIFT_DEFAULT)
        r_img = self.slide.get_img(data.get("right_img")).scale(1).shift(RIGHT * 3.5 + DOWN * Theme.SHIFT_DEFAULT)
    ...
    def render_bullets(self, data):
        ...
        img = self.slide.get_img(data.get("image")).scale(1.2).to_edge(LEFT, buff=Theme.BUFF_LARGE)
    ...
    def render_grid(self, data):
        header = Text(data["text"], color=Theme.PRIMARY).to_edge(UP)
        images = Group(*[self.slide.get_img(img_path).scale(Theme.IMAGE_SCALE_DEFAULT) for img_path in data.get("images", [])])
    ...
    def render_grid_seperate_image(self, data):
        ...
        for i, img_path in enumerate(data.get("images", [])):
            if i < len(positions):
                img = self.slide.get_img(img_path).scale(Theme.IMAGE_SCALE_DEFAULT).to_edge(positions[i], buff=Theme.BUFF_LARGE)
